...
type ConfigPreviewRouterOptions =
  | {
      readonly configCatalogPreview: false
      readonly configHistoryPreview: false
    }
  | ({
      readonly configPreviewDraft: ConfigPreviewDraftHandoff
    } & (
      | { readonly configCatalogPreview: true; readonly configHistoryPreview: false }
      | { readonly configCatalogPreview: false; readonly configHistoryPreview: true }
      | { readonly configCatalogPreview: true; readonly configHistoryPreview: true }
    ))
...
function routes(options: AppRouterOptions): RouteRecordRaw[] {
  return baseRoutes.map((route) => {
    ...
    if (import.meta.env.MODE !== 'production' && options.configHistoryPreview) {
      children.push({
        path: 'preview/config-history',
        name: 'config-history-preview',
        component: () => import('../features/settings/ConfigHistoryPreviewView.vue'),
        props: { configPreviewDraft: options.configPreviewDraft },
        meta: {
          sessionAccess: 'authenticated',
          focusTarget: 'shell-content',
          navigation: {
            labelKey: 'navigation.configHistoryPreview',
            order: 39.5,
            source: MOCK_SOURCE,
          },
        },
