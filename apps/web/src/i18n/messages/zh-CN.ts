const zhCN = {
  ...
  settingsConfig: {
    ...
    preparePublish: '准备发布',
    prepareDelete: '准备删除',
    rollbackVersion: '回滚源版本',
    rollbackVersionHint: '仅接受显式输入的正十进制安全整数；不会查询或推断历史版本。',
    rollbackVersionRequired: '请输入有效的正十进制安全整数版本。',
    prepareRollback: '准备回滚',
    reading: '正在读取配置…',
    publishing: '正在发布配置…',
    deleting: '正在删除配置…',
    'rolling-back': '正在回滚配置…',
    revealValue: '显示敏感 value',
    hideValue: '隐藏敏感 value',
    published: 'RSS 已确认发布 {key}，版本 {version}。',
    deleted: 'RSS 已确认删除 {key}。',
    rolledBack: 'RSS 已确认将 {key} 从源版本 {sourceVersion} 回滚为新版本 {version}。',
    publishUnknown: '发布结果未知；敏感草稿已清除。请显式读取服务端状态后再决定。',
    rollbackUnknown: '回滚结果未知；请显式读取服务端状态后再决定。',
    reconcile: '重新读取服务端状态',
    confirmTitle: '确认 Config 操作',
    confirmPublish: '确认向 {key} 发布当前敏感 value？确认后草稿会立即清除。',
    confirmDelete: '确认删除 {key}？删除响应只确认命令完成。',
    confirmRollback: '确认将 {key} 回滚到显式源版本 {toVersion}？不会预览历史值。',
    cancel: '取消',
    confirm: '确认',
    catalogDraftNotice:
  ...
  secretReferencePublish: {
    ...
    subtitle: '只向 RSS 发布显式外部 store 引用；浏览器不会读取或展示 secret 内容。',
    draftTitle: 'Manual 引用草稿',
    key: 'Secret 配置 key',
    keyHint: '按输入的原始字节发送；浏览器不 trim 或推断格式，RSS 是最终校验方。',
    storeId: 'Store ID',
    storeIdHint: '敏感 store 坐标；仅存在于当前表单和一次请求中。',
    refKey: 'Reference key',
    refKeyHint: '敏感引用坐标；仅存在于当前表单和一次请求中。',
    refVersion: 'Reference version（可选）',
    refVersionHint: '留空时请求中省略此字段；浏览器不推断版本。',
    required: '此字段为必填项。',
    reveal: '显示引用坐标',
    hide: '隐藏引用坐标',
    prepare: '准备发布 Secret 引用',
    publishing: '正在发布 Secret 引用；敏感字段已清除…',
    published: 'RSS 已确认 Secret 引用发布：{key}，版本 {version}。',
    unknown:
      '发布结果未知；全部引用字段已清除，本页不会重试或重置。请先通过可信服务端渠道核验，再离开并重新开始。',
    confirmTitle: '确认发布 Secret 引用',
    confirmDescription:
      '这是不可自动重放的写操作。确认后全部输入会立即清除，浏览器不会读取 secret 内容。',
    cancel: '取消',
    confirm: '确认并发布一次',
  },
  secretMaterialReveal: {
    title: 'Secret Material Reveal',
    subtitle: '从 RSS fresh resolve 一次高风险 Secret material；与引用发布完全分离。',
    draftTitle: 'Manual resolve 草稿',
    key: 'Secret 配置 key',
    keyHint: 'key 仅用于一次显式读取；不会进入页面 URL、历史或持久化。',
    required: '请输入 Secret 配置 key。',
    prepare: '准备危险 Reveal',
    resolving: '正在从 RSS 读取 Secret material；key 已清除…',
    activeTitle: 'Active Secret Material View',
    activeWarning: '高风险：下方是 RSS 返回的原始 Base64 material，不是 UTF-8 文本。',
    lease: '此 active view 将在 30 秒后自动移除；离页、隐藏页面或按 Escape 会立即移除。',
    materialLabel: '原始 Base64 Secret material',
    clipboardWarning:
      '复制会把 material 交给系统剪贴板；页面隐藏或到期不能撤回剪贴板内容，请按设备策略及时处置。',
    copy: '复制 Base64 到系统剪贴板',
    hide: '立即隐藏并释放页面引用',
    copied: 'Base64 material 已复制到系统剪贴板。',
    copyFailed: '无法复制；未显示底层剪贴板错误。',
    expired: 'Secret material 已到期并从 active view 移除。',
    cleared: 'Secret material 已从 active view 移除。',
    confirmTitle: '确认高风险 Secret Material Reveal',
    confirmDescription:
      '确认后将向 RSS 发出一次 fresh resolve，并清除 key。原始 Base64 只在 active view 中显示 30 秒；系统剪贴板和浏览器自身可能保留独立副本，页面不声称能物理擦除 JavaScript 字符串内存。',
    cancel: '取消',
    confirm: '确认并 Reveal 一次',
  },
  configCatalogPreview: {
    title: 'Config Catalog Preview',
    subtitle: '仅展示本地合成 key 元数据，不是 RSS 目录或存在性事实。',
  ...
  policies: {
    ...
    write: {
      ...
      reconciled: '已重新读取服务端版本 {version}；草稿仍保留，请人工比较后再提交。',
      reload: '重新读取服务端状态',
    },
  },
} as const
