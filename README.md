...
Identity 提供独立 Node/Nginx 镜像：`pnpm image:identity --tag rss-identity-web:my-version --build-arg RSS_IDENTITY_WEB_REVISION=$(/usr/bin/git rev-parse HEAD) .`。部署挂载和导航暂不可用行为见[宿主认证 UI](docs/architecture/20260917-2368-embedded-identity.md)。
