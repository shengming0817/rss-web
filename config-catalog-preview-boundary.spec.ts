...
describe('Config Catalog Preview boundary', () => {
  ...
  it('uses one exact build-time gate and an in-memory key-only handoff', () => {
    ...
    const handoff = read('apps/web/src/features/settings/config-catalog-draft-context.ts')
    ...
    expect(handoff).not.toMatch(/value|version|tenant|authorization|SettingsApi|HttpTransport/)
  })
})
